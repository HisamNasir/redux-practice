...
const CartPage = () => {
  ...
  return (
          ...
          <li key={index} className="cart-item mx-auto my-2 w-full flex items-center justify-between bg-white dark:bg-gray-800  rounded-lg shadow-lg p-3">
            <div className='flex items-center'>
             ...
             <div className="cart-item-details">
               <h2>{product.title}</h2>
               <p>Price: <span className='text-sky-500'>${product.price}</span></p>
            ...
            <div className="cart-item-checkbox flex justify-end">
             ...
             <button onClick={() => handleDelete(index)} className="cart-item-delete  font-semibold  tracking-wide shadow-md shadow-black p-2 flex bg-sky-500  hover:bg-opacity-80  text-sm items-center md:text-sm rounded-lg gap-2">
               Remove
             </button>
             </div>
          </li>
    ...
    <div className=" h-[62px] fixed w-full font-semibold px-2 md:px-8 bg-grey-800 bottom-0 border-t-[1px] border-gray-500 dark:bg-gray-800 bg-slate-200">
      ...
      <p className='flex items-center'>Total Price for Selected Items: $<span className=' text-sky-500 text-xl p-2'>{calculateTotalPrice()}</span></p>
